...
def _collect_span_style_stats(ast) -> Tuple[Counter, Counter]:
    ...
    Args:
        ast (fitz.Document): pymupdf Document 객체
...
def analize_morphemes(text: str, topk: int, min_token: int = 50) -> List[str]:
    ...
    del results, tokens, nouns, bigrams, freq

    return top_terms


def normalize_synonym_tokens(ast, syn_map):
    """
    동의어 정규화: syn_map 의존
    """
